...
class TestDeanonymization(unittest.TestCase):
    def test_deanonymize_output(self):
        output = "<StudentNetID1> is a student."
        recognized_tokens = [
            RecognizerResult(entity_type="AnonymizedToken", start=0, end=15, score=1.0)
        ]
        regex_vault = {"<StudentNetID1>": "student123"}
...
class TestAnonymization(unittest.TestCase):
    def test_anonymize_input(self):
        ...
        self.assertIsInstance(result, EngineResult)
        self.assertIn("<StudentNetID1>", result.text)
        self.assertIn("<NNumber1>", result.text)
        self.assertIn("<StudentNetID1>", regex_vault)
        self.assertIn("<NNumber1>", regex_vault)
